...
# 基于指数加权平均来进行center更新
def get_center_momentum(C_feat, label, nCenter, sep_weight=0.1, radius=40, beta=0.99):
    ...
    with torch.no_grad():
        ...
        if torch.cuda.is_available():
            label = label.cuda()
            Center = Center.cuda()
            CenterMean = CenterMean.cuda()
            nCenter = nCenter.cuda()

        if torch.all(nCenter == 0):
...
class gCL(nn.Module):
    ...
    def __init__(self, loss_weight=0.002, sep_weight=0.1, radius=40, alpha=40, rho=0.05):
...
class FasterRCNNTrainer(nn.Module):
    def __init__(self, model_train, optimizer, loss_weight, rho, freeze_epoch):
        ...
        self.rpn_sigma      = 1
        self.roi_sigma      = 1
        self.gCL_loss = gCL(loss_weight=loss_weight, rho=rho)
    ...
    def forward(self, imgs, bboxes, labels, scale, nCenter, epoch):
        n           = imgs.shape[0]
        ...
        roi_cls_locs, roi_scores, features = self.model_train([base_feature, sample_rois, sample_indexes, img_size], scale=1., mode='head')
        features = features.view(n,-1,features.size(1))
        for i in range(n):
            # ------------------------------------------------------ #
            ...
            nCenter = get_center_momentum(feature, gt_roi_label, nCenter)
            if epoch >= self.Freeze_epoch:
                useCenter = nCenter
                roi_cls_loss = nn.CrossEntropyLoss()(roi_score, gt_roi_label) + self.gCL_loss(feature,gt_roi_label,useCenter)
            else:
    ...
    def train_step(self, imgs, bboxes, labels, scale, nCenter, epoch, fp16=False, scaler=None):
        self.optimizer.zero_grad()
        if not fp16:
            losses = self.forward(imgs, bboxes, labels, scale, nCenter, epoch)
            losses[-1].backward()
            self.optimizer.step()
        else:
