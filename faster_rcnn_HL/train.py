...
import argparse
# loss_weight=0.002, sep_weight=0.1, radius=40, alpha=40, rho=0.05
parser = argparse.ArgumentParser(description='PyTorch HL Training')
parser.add_argument('--loss_weight', default=0.003, type=float, help='weight of the constrainted center loss')
parser.add_argument('--sep_weight', default=0.1, type=float, help='weight of the inter-class seperated constraints')
parser.add_argument('--rho', default=0.05, type=float,  help='0-1, the size of general center, 1 denotes the general center is the largest')
parser.add_argument('--radius', default=40, type=int, help='the radius of the hypersphere')
...
if __name__ == "__main__":
    for seed in seeds:
        ...
        #----------------------------------------------------------------------------------------------------------------------------#
        model_path      = '/home/ubuntu/public2/cjp/OD/faster-rcnn-HL/logs/ep050-loss1.349-val_loss1.478.pth'
        #------------------------------------------------------#
        ...
        #   获得图片路径和标签
        #----------------------------------------------------#
        train_annotation_path   = '/home/ubuntu/public2/cjp/OD/faster-rcnn-HL/2007_train.txt'
        val_annotation_path     = '/home/ubuntu/public2/cjp/OD/faster-rcnn-HL/2007_val.txt'

        #----------------------------------------------------#
        #   获取classes和anchor
        #----------------------------------------------------#
        class_names, num_classes = get_classes(classes_path)
        nCenter = torch.zeros(21, 2048)
        ...
        train_util      = FasterRCNNTrainer(model_train, optimizer, loss_weight=args.loss_weight, rho=args.rho, freeze_epoch=Freeze_Epoch)
        #----------------------#
        #   记录eval的map曲线
        #----------------------#
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            fit_one_epoch(model, train_util, loss_history, eval_callback, optimizer, nCenter, epoch, epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, fp16, scaler, save_period, save_dir, seed)
