...
from get_classify_acc import get_acc
import csv
import datetime
...
def test(model_path, seed):
    ...
    #   默认指向根目录下的VOC数据集
    # -------------------------------------------------------#
    VOCdevkit_path = '/home/ubuntu/public2/cjp/OD/VOCdevkit'
    # -------------------------------------------------------#
    #   结果输出的文件夹，默认为map_out
    # -------------------------------------------------------#
    ...
    if map_mode == 0 or map_mode == 1:
        print("Load model.")
        frcnn = FRCNN(confidence=confidence, nms_iou=nms_iou, model_path=model_path)
    ...
    if map_mode == 0 or map_mode == 3:
        ...
        print("Get map done.")
        acc = get_acc()
        log_path = '/home/ubuntu/public2/cjp/OD/faster-rcnn-HL/logs/experimental_HL_logs.csv'
        if not os.path.exists(log_path):
            table_head = [['time', 'seed', 'mAP', 'acc']]
            write_csv(log_path, table_head, 'w+')

        time = datetime.datetime.now().strftime('%Y%m%d-%H%M%S')  # 获取当前系统时间
        a_log = [{'time': time, 'seed': seed, 'mAP': mAP, 'acc': acc}]
