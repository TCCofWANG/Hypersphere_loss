# -*- coding: utf-8 -*-
"""
Created on Mon Nov  7 19:18:55 2022
...
class PyramidNet(nn.Module):
    ...
    def __init__(self, dataset, depth, alpha, feat_dim, bottleneck=False):
        ...
        if self.dataset.startswith('cifar'):
            ...
            # self.fc = nn.Linear(self.final_featuremap_dim, feat_dim)
            self.fc = nn.Sequential(
                    nn.Linear(self.final_featuremap_dim, feat_dim),
...
def PyramidNet110(feat_dim):
    return PyramidNet('cifar10',110,64,feat_dim,False)
    # return PyramidNet(dataset, depth, alpha, feat_dim, bottleneck=False)
    
def PyramidNet164(feat_dim):
    return PyramidNet('cifar100',164,48,feat_dim,True)

def PyramidNet200(feat_dim):
    return PyramidNet('imagenet',200,300,feat_dim,True)

if __name__ == '__main__':
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    net = PyramidNet110(256).to(device)
